...
namespace impl
{
  auto constexpr prepositions =
      std::array{"a", "an", "and", "as", "at", "by", "for", "in", "of", "on", "or", "the", "to"};
  auto constexpr keywords = std::array{"noexcept"};
  auto constexpr type_names = std::array{"unordered_map"};
  ...
  {
    return std::any_of(cbegin(type_names), cend(type_names), [&](auto type_name) {
      return is_prefix(suspect, type_name);
    });
  }
...
#define KAWAII(name) cute::test((&name), go_full_kawaii(#name))
